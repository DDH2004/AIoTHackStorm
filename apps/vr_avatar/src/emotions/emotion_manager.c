...
static const emotion_face_params_t* sg_emotion_params[] = {
    &EMOTION_PARAMS_NEUTRAL,
    &EMOTION_PARAMS_HAPPY,
    &EMOTION_PARAMS_SAD,
    &EMOTION_PARAMS_ANGRY,
...
{
    sg_current_emotion = (sg_current_emotion + 1) % EMOTION_MAX;
    return sg_current_emotion;
}
...
void emotion_draw(emotion_type_t emotion, uint16_t screen_width, uint16_t screen_height)
{
    ...
    if (eye_ry < 3) eye_ry = 3;  // Minimum eye height
